...
namespace fortiss {
    namespace opcua {
        ...
        private:
            std::string message_;
        public:
            explicit NodeNotFoundException(const std::string &nodeId, const std::string &additionalInfo = "") {
        ...
        public:
            explicit StatusCodeException(UA_StatusCode code, const std::string &msg = ""): code(code) {
        ...
        inline std::future<PROMISE_TYPE> setPromiseErrorException(std::promise<PROMISE_TYPE> *promise, UA_StatusCode code) {
            try {
        ...
        inline UA_StatusCode
        UA_ClientConfig_setUriName(UA_ClientConfig *conf, const char *uri, const char *name) {
            // delete pre-initialized values
        ...
        inline const std::shared_ptr<UA_NodeId> UA_Server_getObjectChildId(UA_Server *server, const UA_NodeId objectId,
        ...
        UA_Server_getObjectComponentId(UA_Server *server, const UA_NodeId objectId,
        ...
        inline const std::shared_ptr<UA_NodeId>
        UA_Server_getObjectComponentId_or_Null(UA_Server *server, const UA_NodeId objectId,
                                       const UA_QualifiedName componentName) {
            try {
                return UA_Server_getObjectChildId(server, objectId, componentName,
                                                  UA_NODEID_NUMERIC(0, UA_NS0ID_HASCOMPONENT));
        ...
        inline const std::shared_ptr<UA_NodeId>
        UA_Server_getObjectPropertyId(UA_Server *server, const UA_NodeId objectId,
        ...
        namespace {
            ...
            UA_StatusCode
            UA_Server_nodeIterFindChildOfType(UA_NodeId childId, UA_Boolean isInverse, UA_NodeId referenceTypeId,
                                              void *handle) {
        ...
        inline bool UA_Server_findChildOfType(UA_Server *server, const std::shared_ptr<spdlog::logger> &logger,
                                              const UA_NodeId parent, const UA_NodeId &nodeTypeDefinition,
                                              UA_NodeId *foundNodeId) {
        ...
        inline bool UA_NodeId_parse(const std::string& nodeId, UA_NodeId* id) {
            UA_NodeId_clear(id);
        ...
        inline UA_StatusCode UA_Client_getNamespacesMap(UA_Client *client, const std::shared_ptr<spdlog::logger> &logger,
        ...
        inline bool UA_Client_findChildOfType(UA_Client *client, const std::shared_ptr<spdlog::logger> &logger,
                                       const UA_NodeId parent, const UA_NodeId &nodeTypeDefinition,
                                       UA_NodeId *foundNodeId) {
        ...
        inline bool UA_Client_nodeHasInterface(UA_Client *client, const std::shared_ptr<spdlog::logger> &logger,
                                               const UA_NodeId parent, const UA_NodeId &interfaceTypeDefinition) {
            UA_BrowseRequest bReq;
            UA_BrowseRequest_init(&bReq);
            bReq.requestedMaxReferencesPerNode = 0;
            bReq.nodesToBrowse = UA_BrowseDescription_new();
        ...
        inline bool UA_Client_findChildHavingInterface(UA_Client *client, const std::shared_ptr<spdlog::logger> &logger,
                                       const UA_NodeId parent, const UA_NodeId &interfaceTypeDefinition,
                                       UA_NodeId *foundNodeId) {
            UA_NodeId_clear(foundNodeId);
        ...
        inline bool UA_Client_findChildrenOfType(UA_Client *client, const std::shared_ptr<spdlog::logger> &logger,
                                       const UA_NodeId parent, const UA_NodeId &nodeTypeDefinition,
                                       UA_NodeId **foundNodeId, size_t *foundNodeIdSize) {
            UA_BrowseRequest bReq;
            UA_BrowseRequest_init(&bReq);
            bReq.requestedMaxReferencesPerNode = 0;
            bReq.nodesToBrowse = UA_BrowseDescription_new();
        ...
        inline bool UA_Client_findChildrenImplementingInterfaceRecursively(UA_Client *client, const std::shared_ptr<spdlog::logger> &logger,
                                                     const UA_NodeId *parentNodes, size_t parentNodesSize, const UA_NodeId &interfaceTypeId,
        ...
        inline void UA_Client_findChildrenWithReferenceType(UA_Client *client, const std::shared_ptr<spdlog::logger> &logger,
                                       const UA_NodeId parent, const UA_NodeId &referenceTypeDefinition,
                                       UA_NodeId **foundNodeId, size_t *foundNodeIdSize) {
            UA_BrowseRequest bReq;
            UA_BrowseRequest_init(&bReq);
            bReq.requestedMaxReferencesPerNode = 0;
        ...
        inline bool UA_Client_findChildWithBrowseName(UA_Client *client, const std::shared_ptr<spdlog::logger> &logger,
                                               const UA_NodeId &parentNode, const UA_QualifiedName &browseName,
                                               UA_NodeId *foundNodeId) {
            UA_NodeId_clear(foundNodeId);
        ...
        inline bool UA_Client_findBrowsePath(UA_Client *client, const std::shared_ptr<spdlog::logger> &logger,
                                                      const UA_NodeId &parentNode, std::vector<UA_QualifiedName> &browsePath,
                                                      UA_NodeId *foundNodeId) {
            UA_NodeId_clear(foundNodeId);
        ...
        inline bool UA_Client_findChildWithName(UA_Client *client, const std::shared_ptr<spdlog::logger> &logger,
                                                      const UA_NodeId &parentNode, const UA_String &name,
                                                      UA_NodeId *foundNodeId) {
            UA_NodeId_clear(foundNodeId);
        ...
        inline UA_UInt16 UA_Server_getNamespaceIdByName(UA_Server *uaServer, const char *nsid) {
            size_t nsIdTmp = 0;
        ...
        struct serverOnNetworkCallbackData {
            ...
            std::function<void(const UA_ServerOnNetwork *, UA_Boolean isServerAnnounce)> onServerAnnounce;
        };
        ...
        inline void UA_Server_onServerAnnounce(UA_Server *uaServer, const serverOnNetworkCallbackData& data) {
        ...
        inline UA_StatusCode UA_Server_setPeriodicRegister(
           ...
           * currently uses encrpytion optionally */
            ...
            UA_EndpointDescription *endpointRegister = UA_Helper_getRegisterEndpointFromServer(discoveryUrl.c_str(), logger, !clientCertPath.empty() && !clientKeyPath.empty());
            if(endpointRegister == NULL || endpointRegister->securityMode == UA_MESSAGESECURITYMODE_INVALID) {
                logger->error("Could not find any suitable endpoints on discovery server");
                return UA_STATUSCODE_BADINVALIDARGUMENT;
            }

            *clientRegister = UA_Helper_getClientForEndpoint(endpointRegister, logger, clientCertPath, clientKeyPath, clientAppUri, clientAppName);
            if(!*clientRegister) {
                logger->error("Could not create the client for remote registering");
            ...
            UA_StatusCode retval = UA_Server_addPeriodicServerRegisterCallback(uaServer, *clientRegister, endpointUrl,
                                                                 10 * 60 * 1000, 500, periodicCallbackId);
            UA_free(endpointUrl);
            if(retval != UA_STATUSCODE_GOOD) {
                logger->error("Could not create periodic job for server register. StatusCode %s",
        ...
        initServerConfig(
            ...
            config->discovery.mdnsEnable = enableMdns;
        ...
        static UA_StatusCode powerOffDeviceCallback(UA_Server *server,
        ...
        inline
        UA_StatusCode setPowerOffHandler(UA_Server* server, const UA_NodeId& deviceNodeId, powerOffDeviceCallbackData powerOffData) {
        ...
        getEndpointWithHighestSecurity(const char *discoveryServerUrl, const std::shared_ptr<spdlog::logger>& logger) {
            UA_Client *client = UA_Client_new();
            UA_ClientConfig *clientConfig = UA_Client_getConfig(client);
            clientConfig->logger.log = fortiss::log::opcua::UA_Log_Spdlog_log;
            clientConfig->logger.context = logger.get();
        ...
        getBestEndpointFromServer(
                const std::vector<std::string>& discoverUrls,
