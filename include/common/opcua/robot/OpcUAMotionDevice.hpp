        ...
        namespace robot {
            ...
            public:
                OpcUAMotionDevice(
                        ...
                        const UA_NodeId& motionDeviceNodeId,
            ...
            private:
                ...
                UA_StatusCode initializeNodeIds(UA_NodeId motionDeviceId) {
                    ...
                    std::shared_ptr<UA_NodeId> flangeToolClearMethodId = UA_Server_getObjectComponentId(server, motionDeviceId, UA_QUALIFIEDNAME(forRobNsId,
                                                                                                                                                 const_cast<char*>("FlangeToolClear")));
                    std::shared_ptr<UA_NodeId> flangeToolSetMethodId = UA_Server_getObjectComponentId(server, motionDeviceId, UA_QUALIFIEDNAME(forRobNsId,
                                                                                                                                               const_cast<char*>("FlangeToolSet")));


                    LockedServer ls = server->getLocked();
                    UA_StatusCode retval = UA_Server_setNodeContext(ls.get(), *flangeToolClearMethodId, this);
                    if (retval != UA_STATUSCODE_GOOD) {
                        return retval;
                    }

                    retval = UA_Server_setNodeContext(ls.get(), *flangeToolSetMethodId, this);
                    if (retval != UA_STATUSCODE_GOOD) {
                        return retval;
                    }

                    retval = UA_Server_setMethodNode_callback(ls.get(), *flangeToolClearMethodId, &OpcUAMotionDevice::flangeToolClearMethodCallback);
                    if (retval != UA_STATUSCODE_GOOD) {
                        throw std::runtime_error("Adding flangeToolClear method failed");
                    }
                    retval = UA_Server_setMethodNode_callback(ls.get(), *flangeToolSetMethodId, &OpcUAMotionDevice::flangeToolSetMethodCallback);
                    if (retval != UA_STATUSCODE_GOOD) {
                ...
                void initializeNodeIdAxisMap(UA_NodeId motionDeviceId) {
                    ...
                    std::shared_ptr<UA_NodeId> axesId = UA_Server_getObjectComponentId(server, motionDeviceId,
                                                                                       UA_QUALIFIEDNAME(robNsId, const_cast<char*>("Axes")));
                    ...
                    for (unsigned i = 1; i <= AXIS_COUNT; i++) {
                        ...
                        LockedServer ls = server->getLocked();
                        auto positionBpr = UA_Server_translateBrowsePathToNodeIds(ls.get(), &tmpBp);
                        ...
                        // set nodeContext
                        UA_StatusCode retval = UA_Server_setNodeContext(ls.get(), positionBpr.targets[0].targetId.nodeId,
                                                                        this);
                        if (retval != UA_STATUSCODE_GOOD) {
                            UA_BrowsePathResult_deleteMembers(&positionBpr);
                ...
                static UA_StatusCode flangeToolSetMethodCallback(
                        UA_Server* server,
                        const UA_NodeId* sessionId,
                        void* sessionHandle,
                ...
                static UA_StatusCode flangeToolClearMethodCallback(
                        UA_Server* server,
                        const UA_NodeId* sessionId,
                        void* sessionHandle,
                        const UA_NodeId* methodId,
